    ...
    with tqdm(total=len(train_dataloader) * epochs) as pbar:
        ...
        for epoch in range(epochs):
            ...
            if not epoch % epochs_till_chkpt:
                tqdm.write(f'Epoch {epoch + 1}, loss: {np.mean(epoch_loss.cpu().numpy())}')
            ...
            if validation_dataloader != None:
                ...
                with torch.no_grad():
                    ...
                    tqdm.write(f'Validation loss after epoch {epoch + 1}: {np.mean(val_losses.cpu().numpy())}')
                model.train()
